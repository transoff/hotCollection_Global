...
HARNESS=dedup_harness.rb DYLD_INSERT_LIBRARIES=... \
  ruby tracer.rb corpus_dedup -artifact_prefix=findings_dedup/ -timeout=25
```
...
Для многопоточных харнессов добавляй `-timeout=25`: дедлок на `ConditionVariable`
иначе повесит прогон навсегда вместо того, чтобы записать находку.
...
### Нормальное завершение на macOS

По истечении `-max_total_time` процесс завершается с кодом **142** (SIGALRM) и
без финальной строки `Done`. Это не сбой: корпус и все `crash-*` уже записаны на
диск по ходу прогона. Настоящая находка — это код **77** и файл в папке находок.
